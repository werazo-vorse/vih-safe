...
// Componente FactCard corregido (sin rotación 3D problemática)
function FactCard({ fact }: { fact: any }) {
  const [isFlipped, setIsFlipped] = useState(false);

  const backgroundColor = fact.isMyth
    ? "bg-red-50 border-red-200"
    : "bg-green-50 border-green-200";
  const badgeColor = fact.isMyth ? "bg-red-500" : "bg-green-600";
  const badgeText = fact.isMyth ? "Mito" : "Verdad";
      ...
      <AnimatePresence mode="wait">
        {!isFlipped ? (
          ...
          >
            <Card className="hover:border-primary/50 transition-colors">
              <CardContent className="p-6 flex flex-col items-center justify-center min-h-[200px] text-center">
                <div className="text-sm font-semibold tracking-wider text-muted-foreground mb-4 uppercase">
                  ¿Mito o Verdad?
                </div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.2 }}
          >
            <Card className={`border-2 ${backgroundColor}`}>
              <CardContent className="p-6 flex flex-col items-center justify-center min-h-[200px] text-center overflow-y-auto">
                <div
                  className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-bold mb-3 text-white ${badgeColor}`}
                >
                  {badgeText}
                </div>
...
export default function EducationModule() {
  ...
  const { data: module, isLoading } = useGetEducationModule(moduleId, {
    query: {
      enabled: !!moduleId,
      queryKey: getGetEducationModuleQueryKey(moduleId),
    },
  });
            ...
            <div className="bg-card border rounded-2xl p-6 md:p-10 shadow-sm">
              <div className="text-center mb-10">
                ...
                <p className="text-muted-foreground">
                  Responde estas rápidas preguntas para evaluar tu comprensión.
                </p>
              </div>
              ...
              {!quizResult ? (
                ...
                <AnimatePresence>
                  ...
                  >
                    <div
                      className={`p-6 rounded-xl border text-center ${quizResult.passed ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}
                    >
                      ...
                      <div className="font-medium">
                        {quizResult.passed
                          ? "¡Excelente trabajo! Has comprendido los conceptos clave."
                          : "Buen intento. Revisa las explicaciones abajo para fortalecer tu conocimiento."}
                      </div>
                    </div>
                    ...
                    <div className="pt-6 flex justify-center">
                      <Link href="/educacion">
                        <Button variant="outline">Volver a Módulos</Button>
                      </Link>
                    </div>
                  </motion.div>
                </AnimatePresence>
