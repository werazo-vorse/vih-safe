...
2. **Chatbot Educativo** — AI assistant in Colombian Spanish with safe, evidence-based answers about HIV/STIs, condoms, PrEP/PEP, treatment (I=I), and Manizales resources. Powered by OpenAI through Replit AI Integrations.
...
- `artifacts/api-server/src/data/questions.ts` — the 19 adapted questions in Colombian Spanish
...
The chatbot uses Replit's AI Integrations proxy (no user API key needed). Env vars `AI_INTEGRATIONS_OPENAI_BASE_URL` and `AI_INTEGRATIONS_OPENAI_API_KEY` are auto-provisioned. Model: `gpt-5.4` with JSON response format and a Spanish system prompt named "VIHSafe".
